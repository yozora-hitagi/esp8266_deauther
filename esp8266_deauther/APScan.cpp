...
bool APScan::start() {
	...
	if (debug)
		Serial.println("Scan results: " + (String) results);

	for (int i = 0; i < results; i++) {
		Mac _ap;
		_ap.set(WiFi.BSSID(i)[0], WiFi.BSSID(i)[1], WiFi.BSSID(i)[2],
...
void APScan::sendResults() {
	if (debug)
		Serial.print("sending AP scan result JSON -> ");

	String json = "{\"aps\":[";

	for (int i = 0; i < results && i < maxAPScanResults; i++) {
		json += "{\"i\":" + (String) i + ",\"c\":" + (String) getAPChannel(i)
				+ ",\"m\":\"" + getAPMac(i) + "\",\"ss\":\"" + getAPName(i)
				+ "\",\"r\":" + (String) getAPRSSI(i) + ",\"e\":"
				+ (String) encryption[i] + ",\"se\":" + (String) isSelected(i)
				+ "}";
		if ((i != results - 1) && (i != maxAPScanResults - 1))
			json += ",";
	}

	String m = settings.multiAPs ? "1" : "0";
	json += "],\"multiAPs\":\"" + m + "\"}";
...
//	if (debug)
//		Serial.print("getting AP scan result JSON ");
//	String json = "{ \"aps\":[ ";
//	for (int i = 0; i < results && i < maxAPScanResults; i++) {
//		if (debug)
//			Serial.print(".");
	...
	for (int i = 0; i < results - 1; i++) {
		...
		for (int h = 0; h < results - i - 1; h++) {
			...
			if (rssi[h] < rssi[h + 1]) {
				...
				Serial.println((String) rssi[h] + " < " + (String) rssi[h + 1]);
		}
	}
}
...
void APScan::select(int num) {
	if (debug)
		Serial.println("select " + (String) num + " - " + !selected[num]);
	if (num < 0) {
		if (num == -1) {
