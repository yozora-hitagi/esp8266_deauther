...
void drawInterface() {
	if (displayOn) {
		...
		int _lrow = 0;
		for (int i = curSite * rowsPerSite - rowsPerSite;
				i < curSite * rowsPerSite; i++) {
			if (i == 0)
				display.drawString(3, i * fontSize, "-> WiFi " + wifiMode);
			else if (i == 1)
				display.drawString(3, i * fontSize, "-> " + scanMode);
			else if (i == 2)
				display.drawString(3, i * fontSize,
						"-> " + attackMode_deauth + " deauth");
			else if (i == 3)
				display.drawString(3, i * fontSize,
						"-> " + attackMode_beacon + " beacon flood");
			else if (i - 4 < apScan.results) {
				display.drawString(4, _lrow * fontSize,
						apScan.getAPName(i - 4));
			...
			if (_lrow == lrow)
				display.drawVerticalLine(0, _lrow * fontSize, fontSize);
			_lrow++;
		}
...
void startWifi() {
	...
	if (settings.password.length() < 8)
		Serial.println("WARNING: password must have at least 8 characters!");
	if (settings.ssid.length() < 1 || settings.ssid.length() > 32)
		Serial.println(
				"WARNING: SSID length must be between 1 and 32 characters!");
	wifiMode = "ON";
}

void stopWifi() {
	Serial.print("stopping WiFi AP");
	//换成2.3后这里异常，也不晓得啥原因，看上去也没啥必要。注释掉。
	//WiFi.disconnect();
	Serial.print(" -> ");
...
void loadFunctionsJS() {
	sendFile(200, "text/javascript", data_functions_js,
			sizeof(data_functions_js));
}
void loadAPScanJS() {
	sendFile(200, "text/javascript", data_apscan_js, sizeof(data_apscan_js));
}
void loadStationsJS() {
	sendFile(200, "text/javascript", data_stations_js,
			sizeof(data_stations_js));
}
void loadAttackJS() {
...
void loadStyle() {
	sendFile(200, "text/css;charset=UTF-8", data_style_css,
			sizeof(data_style_css));
}
...
void startAttack() {
	if (server.hasArg("num")) {
		...
		if (apScan.getFirstTarget() > -1 || _attackNum == 1
				|| _attackNum == 2) {
...
void loop() {
	...
	if (digitalRead(upBtn) == LOW || digitalRead(downBtn) == LOW
			|| digitalRead(selectBtn) == LOW || digitalRead(displayBtn) == LOW) {
		if (canBtnPress) {
			if (digitalRead(upBtn) == LOW)
				...
				buttonPressed = 2;
			else if (digitalRead(displayBtn) == LOW)
				buttonPressed = 3;
			canBtnPress = false;
		}
	} else if (!canBtnPress) {
		...
		if (buttonPressed == 0 && curRow > 0) {
			...
			if (curRow == 0) {
				...
				if (attackMode_deauth == "START"
						&& apScan.getFirstTarget() > -1)
					attack.start(0);
				else if (attackMode_deauth == "STOP")
					attack.stop(0);
				...
				if (attackMode_beacon == "START") {
					...
					if (apScan.selectedSum > 0) {
						...
						for (int i = 0; i < apScan.results; i++) {
							if (apScan.isSelected(i)) {
								ssidList.addClone(apScan.getAPName(i),
										clonesPerSSID,
										apScan.getAPEncryption(i) != "none");
							}
						}
					...
					//start attack
					attack.start(1);
				} else if (attackMode_beacon == "STOP")
					attack.stop(1);
			}
